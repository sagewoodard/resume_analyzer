...
// ---- Helpers to harden against partial feedback ----
type ScoreField = { score?: number; tips?: Tip[]; [k: string]: any };
type Feedback = {
...
function section(f?: ScoreField): { score: number; tips: Tip[] } {
  return {
    score: num(f?.score, 0),
    tips: arr<Tip>(f?.tips, []),
  };
...
const Details = ({ feedback }: { feedback?: Feedback | null }) => {
  const tone = section(feedback?.toneAndStyle);
  const content = section(feedback?.content);
  const structure = section(feedback?.structure);
  const skills = section(feedback?.skills);
